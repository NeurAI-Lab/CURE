...
def collate_loss(args, _loss_main=0, loss_dml=None, loss_adv=0, loss_nat_ce=0, loss_rob_ce=0, loss_rob=0, m1=True, epoch=0):
    ...
    if m1:
        ...
        str = "m2"
    final_loss = {}
    final_loss[str +'_loss_main'] = _loss_main
    final_loss[str + '_loss_nat_ce'] = loss_nat_ce
    final_loss[str + '_loss_rob_ce'] = loss_rob_ce
    final_loss[str + '_loss_rob'] = loss_rob

    final_loss['loss'] = 0

    if loss_dml is not None:
        if 'kl' in args.aux_loss_type:
            loss_wt_kl = args.aux_loss_wt_kl1 if m1 else args.aux_loss_wt_kl2
            final_loss[str +'_loss_kl'] = loss_dml['loss_kl'] * loss_wt_kl
            loss_dml['loss'] += final_loss[str +'_loss_kl']

        if 'l2' in args.aux_loss_type:
            loss_wt_l2 = args.aux_loss_wt_l21 if m1 else args.aux_loss_wt_l22
            final_loss[str +'_loss_l2'] = loss_dml['loss_l2'] * loss_wt_l2
            loss_dml['loss'] += final_loss[str +'_loss_l2']

        if 'at' in args.aux_loss_type:
            loss_wt_at = args.aux_loss_wt_at1 if m1 else args.aux_loss_wt_at2
            final_loss[str +'_loss_at'] = loss_dml['loss_at'] * loss_wt_at
            loss_dml['loss'] += final_loss[str +'_loss_at']

        if 'fitnet' in args.aux_loss_type:
            loss_wt_at = args.aux_loss_wt_at1 if m1 else args.aux_loss_wt_at2
            final_loss[str +'_loss_fitnet'] = loss_dml['loss_fitnet'] * loss_wt_at
            loss_dml['loss'] += final_loss[str +'_loss_fitnet']

        final_loss['loss_ema'] = loss_dml['loss']
        final_loss['loss'] += loss_dml['loss']

    final_loss['loss'] += _loss_main
...
class dml_Loss():
    ...
    def get_loss(self):
        ...
        if 'kl' in self.args.aux_loss_type:
            self.loss_dml['loss_kl'] = 0
            loss = self.kl_loss(self.st_out, self.tch_out, 1.0)
            self.loss_dml['loss_kl'] = loss

        if 'at' in self.args.aux_loss_type:
        ...
        if 'fitnet' in self.args.aux_loss_type:
            self.loss_dml['loss_fitnet'] = 0

            if self.args.model_architecture == 'MLP':
        ...
        if 'l2' in self.args.aux_loss_type:
