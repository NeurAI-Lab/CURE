...
def solver(args):
    ...
    for epoch in tqdm(range(1, args.epochs + 1), desc='training epochs'):
        ...
        saver.save_models(method.model, optimizer, epoch, 0.0)
        ...
        if epoch >= args.epochs_warmup:
            args.reinit = True
            args.adv_mode = "cure_dual"
            args.train_mode = 'cure'
            args.reinit_mode = 'rgp'
            # method.model_ema = deepcopy(method.model).to(device)
            for ema_param, param in zip(method.model_ema.parameters(), method.model.parameters()):
